...
async def test_health_check_cache_expiry():
    ...
    mock_times = [0.0, 1.0, 15.0]  # 0s, 1s (cached), 15s (expired)
    mock_time_iter = iter(mock_times)

    def mock_time():
        return next(mock_time_iter)

    with patch("src.app.main.time.time", side_effect=mock_time):
        async with AsyncClient(
            transport=ASGITransport(app=app),
            base_url="http://test",
        ) as client:
            ...
            # Reset the mock to allow fresh health checks
            mock_time_iter = iter([15.0])
...
async def test_health_check_no_db_session_when_cached():
    ...
    with patch("src.app.main.get_public_session") as mock_session:
